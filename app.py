...
# ============================================================
# ÜYELİK VE GİRİŞ SİSTEMİ (AUTHENTICATION)
# ============================================================
# Şifreleri açık metin olarak değil, güvenli hash olarak saklamak için fonksiyon
def hash_password(password):
    return hashlib.sha256(str.encode(password)).hexdigest()

# Veritabanını (SQLite) başlatma ve tablo oluşturma fonksiyonu
def init_db():
...
init_db() # Uygulama açıldığında veritabanını kontrol et/oluştur

# Kullanıcı oturum durumunu (Session State) kontrol et
if "logged_in" not in st.session_state:
    st.session_state["logged_in"] = False
    st.session_state["username"] = ""

# EĞER KULLANICI GİRİŞ YAPMADIYSA SADECE BU EKRANI GÖSTER
if not st.session_state["logged_in"]:
    st.markdown("<h2 style='text-align: center; color: #1A5276;'>Sisteme Giriş / System Login</h2>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    # 1. GİRİŞ YAP SEKMESİ
    with tab_login:
        l_user = st.text_input("Kullanıcı Adı / Username", key="l_user")
        l_pass = st.text_input("Şifre / Password", type="password", key="l_pass")
        
        if st.button("Giriş Yap / Login", type="primary"):
            conn = sqlite3.connect('users.db')
            c = conn.cursor()
            c.execute("SELECT * FROM users WHERE username=? AND password=?", (l_user, hash_password(l_pass)))
            result = c.fetchone()
            conn.close()
            ...
            if result:
                ...
                st.rerun() # Sayfayı yenile ve alt kısımdaki dashboard'u aç
            else:
                st.error("Kullanıcı adı veya şifre hatalı! / Invalid username or password!")
                
    # 2. KAYIT OL SEKMESİ
    with tab_register:
        r_user = st.text_input("Belirlediğiniz Kullanıcı Adı / Choose Username", key="r_user")
        r_email = st.text_input("E-posta Adresi / Email Address", key="r_email")
        r_pass = st.text_input("Şifre / Password", type="password", key="r_pass")
        r_pass_conf = st.text_input("Şifreyi Onayla / Confirm Password", type="password", key="r_pass_conf")
        
        if st.button("Kayıt Ol / Register", type="primary", key="btn_register"):
            if r_pass != r_pass_conf:
                st.error("Şifreler uyuşmuyor! / Passwords do not match!")
            elif not r_user or not r_pass:
                st.error("Lütfen tüm alanları doldurun! / Please fill in all fields!")
            else:
                conn = sqlite3.connect('users.db')
                c = conn.cursor()
                c.execute("SELECT * FROM users WHERE username=?", (r_user,))
                if c.fetchone():
                    st.error("Bu kullanıcı adı zaten alınmış! / Username already taken!")
                else:
                    c.execute("INSERT INTO users (username, password, email) VALUES (?, ?, ?)", 
                              (r_user, hash_password(r_pass), r_email))
                    conn.commit()
                    st.success("Kayıt başarılı! 'Giriş Yap' sekmesinden sisteme girebilirsiniz. / Registration successful! You can now log in.")
                conn.close()
                
    # ÇOK ÖNEMLİ: Eğer giriş yapılmadıysa, Streamlit kodun aşağısını (Dashboard'u) okumayı BURADA DURDURUR.
    st.stop() 


# EĞER KULLANICI GİRİŞ YAPTIYSA, YUKARIDAKİ st.stop() ÇALIŞMAZ VE KOD BURADAN AŞAĞIYA DEVAM EDER.
# Mevcut Dashboard kodlarınız çalışmaya başlar.

# Sadece Sidebar'a aktif kullanıcıyı ve bir çıkış butonu ekleyelim:
st.sidebar.markdown(f"👤 **Aktif Kullanıcı:** {st.session_state['username']}")
if st.sidebar.button("🚪 Çıkış Yap / Logout"):
    st.session_state["logged_in"] = False
    st.session_state["username"] = ""
    st.rerun()
